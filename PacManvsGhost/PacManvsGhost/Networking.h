...
class Networking
{
	...
	// This class is to handle all of the data that Pacman will need to send and receive to the server.
...
public:
	...
	// Game functions to receive/send to the server
	void ReceivePositionPacket(sf::Packet& receivedPacket, float dt, Ghosts& ghosts);	// Receives Position of other player
	...
	void SendPositionPacket(sf::Packet& packet, PacMan& pm);							// Send Players Position to server
